...
function [z,res] = fast_gcd_wls(f,g,delta)
disp(f);
disp(g);
...
dU=c_gs_gecp(f,g); % diagonal of the U factor
eta=delta_c*sqrt(N);
k=N;
...
if tdeg==m
    z=g;
    dfg=div_fft(f,g,delta);
    f_nearness=[norm(f-conv(dfg,z)) 0];
    [z,res]=c_f_newton_iter(f,g,z,dfg,1,f_nearness,1);
    if res<[delta delta]
        return
    end
